...
export default function App() {
	// 모달의 상태가 변경될때마다 재랜더링해서 모달을 닫거나 띄우므로 useState
	...
	return (
		<>
			...
			<Modal
				...
				onRequestClose={() => {
					setModal02IsOpen(false); // close 버튼이 아닌 모달의 밖을 클릭해도 닫힌다.
				}}
			...
			<Modal
				...
				onRequestClose={() => {
					setModal03IsOpen(false); // close 버튼이 아닌 모달의 밖을 클릭해도 닫힌다.
				}}
				shouldCloseOnOverlayClick={false} // false이면 onRequestClose가 작동X
			...
			<button onClick={() => setModal04IsOpen(true)}>
				modal04
			</button>
			<br />
			<br />
			...
			<button onClick={() => setModal05IsOpen(true)}>
				modal05
			</button>
			<br />
			<br />
